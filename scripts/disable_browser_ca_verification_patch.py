...
def main():
    ...
    os.system(f'{modules.hactoolnet} --keyset {args.prod_keys} --intype switchfs --raw {args.firmware} --title 0100000000000803 --romfsdir titleid/0100000000000803/romfs/')

    with open('titleid/0100000000000803/romfs/nro/netfront/core_2/default/cfi_enabled/webkit_wkc.nro.lz4', 'rb') as file:
        input_data = file.read()
        decompressed = lz4.block.decompress(input_data)

    with open('titleid/0100000000000803/romfs/uncompressed_browser_ssl.nro', 'wb') as decompressed_browser_file:
    ...
    with open(f'./patches/atmosphere/nro_patches/disable_browser_ca_verification/{ips_hash}.ips', 'wb') as text_file:
        text_file.write(bytes.fromhex(modules.IPS32_HEADER + ips_patch1 + ips_patch2 + modules.IPS32_FOOTER))
